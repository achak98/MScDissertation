...
def encoding(essay_list, tokenizer):
    ...
    for essay in tqdm(essay_list):
        ...
        encodings.append(np.squeeze(np.asarray(encoded_input)))
...
essay_encodings = encoding(dataset["essay"], roberta, tokenizer)
...
for n, (train, test) in enumerate(kf.split(dataset)):
    ...
    train_loader = get_loader(train_df, id2emb, essay_encodings, shuffle=True)
    print("getting test loader")
    test_loader = get_loader(test_df, id2emb, essay_encodings, shuffle=False)
