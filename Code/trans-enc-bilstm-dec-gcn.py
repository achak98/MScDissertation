...
from transformers import DebertaModel, LongformerModel, LongformerDecoder, LongformerTokenizer
...
class GCNLSTMMLP(nn.Module):
    def __init__(self, hidden_dim, num_heads):
        super(GCNLSTMMLP, self).__init__()

        self.encoder = DebertaModel.from_pretrained('microsoft/deberta-base')
        self.gcn = GCNConv(hidden_dim, hidden_dim)
        self.attention = MultiheadAttention(hidden_dim, num_heads)
        self.decoder = BiLSTMDecoder(hidden_dim, hidden_dim//4)
        self.biaffine = Biaffine(hidden_dim//4, hidden_dim//16)
...
def train():
    ...
    model = GCNLSTMMLP(hidden_dim, num_classes, num_layers, num_heads, dropout)
