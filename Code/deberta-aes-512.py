...
def mean_encoding(essay_list, model, tokenizer):
  ...
  for essay in tqdm(essay_list):
    encoded_input = tokenizer(essay, padding=True, truncation=True, max_length=config.max_position_embeddings, return_tensors='pt', return_attention_mask=True).to(device)
    with torch.no_grad():
