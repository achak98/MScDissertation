...
class GCNLongformerCRF(nn.Module):
    def __init__(self, hidden_dim, num_heads):
        super(GCNLongformerCRF, self).__init__()

        self.encoder = DebertaModel.from_pretrained('microsoft/deberta-base')
        self.gcn = GCNConv(hidden_dim, hidden_dim)
        self.attention = MultiheadAttention(hidden_dim, num_heads)
        self.decoder = LongformerDecoder.from_pretrained('allenai/longformer-base-4096')
...
def train():
    ...
    model = GCNLongformerCRF(hidden_dim, num_classes, num_layers, num_heads, dropout)
