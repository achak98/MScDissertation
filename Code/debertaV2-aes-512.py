...
if os.path.exists(os.path.join(data_dir,'embeddings_dv2_512.pt')):
    essay_embeddings = torch.load(os.path.join(data_dir,'embeddings_dv2_512.pt'), map_location=torch.device('cpu'))
    print(f"embeddings loaded from {os.path.join(data_dir,'embeddings_dv2_512.pt')}")
else:
    essay_embeddings = mean_encoding(dataset['essay'], roberta, tokenizer)
    torch.save(essay_embeddings, os.path.join(data_dir,'embeddings_dv2_512.pt'), pickle_protocol=4)
